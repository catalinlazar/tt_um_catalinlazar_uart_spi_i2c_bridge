...
// Simple memory-mapped register file, 8 x 8-bit registers.
// addr 0x00 CTRL      : [0]=soft_rst [1]=reserved [2]=i2c_en
// addr 0x01           : unimplemented, always reads 0x00, writes ignored
// addr 0x02 I2C_CFG   : [4:0]=clkdiv [5]=stretch_en(informational)
// addr 0x03 STATUS    : read-only, driven from live status bus
// addr 0x04 IRQ_EN    : unimplemented, always reads 0x00, writes ignored
// addr 0x05 BAUD_LO   : UART baud divisor, bits [7:0]
// addr 0x06 BAUD_HI   : UART baud divisor, bits [15:8]
// addr 0x07            : general purpose scratch register
module regfile (
    input  wire       clk,
    input  wire       rst_n,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (wr_en && addr != 3'd1 && addr != 3'd3 && addr != 3'd4) begin
                regs[addr] <= wr_data;
            end
    ...
    always @(*) begin
        case (addr)
            3'd1:    rd_data = 8'h00;   // unimplemented
            3'd3:    rd_data = status_live;
            3'd4:    rd_data = 8'h00;   // IRQ_EN unimplemented
